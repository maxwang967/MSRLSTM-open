...
def conv_block(input):
    conv1 = Conv1D(64, kernel_size=5, padding='SAME', activation='relu')(input)
    conv1 = Conv1D(64, kernel_size=5, padding='SAME', activation='relu')(conv1)
    return MaxPooling1D(2)(conv1)
...
def build_model():
    ...
    gyrx1 = conv_block(gyrx)
    gyry1 = conv_block(gyry)
    gyrz1 = conv_block(gyrz)
    laccx1 = conv_block(laccx)
    laccy1 = conv_block(laccy)
    laccz1 = conv_block(laccz)
    magx1= conv_block(magx)
    magy1 = conv_block(magy)
    magz1 = conv_block(magz)
    pres1 = conv_block(pres)

    all_inputs = concatenate([gyrx1, gyry1, gyrz1, laccx1, laccy1, laccz1, magx1, magy1, magz1, pres1], axis=2)
    print(all_inputs.shape)

    lstm1 = LSTM(128, input_shape=(225, 640), return_sequences=True)(all_inputs)
